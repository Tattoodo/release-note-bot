...
const isProcessable = ({ action, pull_request }) =>
	processableActions.includes(action) && (isRelease(pull_request) || isStaging(pull_request));
...
const storyUrl = (id) => `https://api.app.shortcut.com/api/v2/stories/${id}?token=${process.env.CLUBHOUSE_API_TOKEN}`;

const fetchStory = async (id) => global.fetch(storyUrl(id)).then((r) => r.json());

const getChangeLog = async ({ owner, repo, pull_number }) => {
	const commits = await octokit.paginate(octokit.pulls.listCommits.endpoint({ owner, repo, pull_number }));
...
const hasMappingJsonChanged = async ({ owner, repo, pull_number }) => {
	const files = await octokit.paginate(octokit.pulls.listFiles.endpoint({ owner, repo, pull_number }));
	return files.some(({ filename }) => mappingJsonFile.test(filename));
};

const stripGeneratedContent = (body) => body.replace(changesRe, '').replace(mappingJsonNoticeRe, '').trim();

const processPullRequest = async ({ organization, repository, number, pull_request }) => {
